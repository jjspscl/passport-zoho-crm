 ...
 *   - `redirect_uri`   URL to which Zoho CRM will redirect the user after granting authorization
 ...
 *         redirect_uri: 'https://www.example.net/auth/zohocrm/callback',
 ...
 */
 function Strategy(options, verify) {
  options = options || {};
  options.authorizationURL = options.authorizationURL || 'https://accounts.zoho.com/oauth/v2/auth?access_type=offline';
  options.tokenURL = options.tokenURL || 'https://accounts.zoho.com/oauth/v2/token';
  options.scopeSeparator = options.scopeSeparator || ',';
  options.customHeaders = options.customHeaders || {};

  if (!options.customHeaders['redirect_uri']) {
    options.customHeaders['redirect_uri'] = options.redirect_uri || 'passport-zoho-crm';
  }
  ...
  this._oauth2.authorizationParams = function(options) {
    return {
